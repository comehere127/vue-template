<template>
  <el-card
    ...
    shadow="never"
    class="box-card promotion-item"
  >
    <div v-show="!isLoading" @click="handlePaymentClick">
      <h3>{{ title }}</h3>
      <div :style="{backgroundImage: 'url('+imgUrl+')'}" class="thumb-img" />
      <!-- <el-button type="primary" round size="small">{{ $t('ApplicationButton') }}</el-button> -->
      <div class="promotion-qty">
        <span>{{ number }}</span>
        Ưu đãi
      </div>
    </div>
